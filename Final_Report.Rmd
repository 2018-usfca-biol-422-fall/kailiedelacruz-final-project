...
melted_obj %>%
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) # Helpful in making the
...
melted_obj %>%
  group_by(env_material, Phylum, smoker) %>% # needed to group by Phylum, smoker,
  # and env_material because this figure only needs data in relation to these columns apart
  # of the melted_obj table.
  summarize(sum_count = sum(Abundance)) %>% # looks at the sum of abundances
  # rather than each individual abundance value
...
# melted_obj was created in a separate R script. The data apart
# of melted_obj was loaded at the beginning of this file.
# This figure takes a closer look at the abundance counts of
...
melted_obj %>%
  group_by(env_material, Phylum, smoker, Family) %>% # needed to group by Phylum, smoker,
  ...
  # looking specifically at smoking individuals and dont need the data in
  # relation to the nonsmokers
  filter(Phylum == "Firmicutes") %>% # need to filter Firmicutes because we want to
  # look at the families that are specifically associated with the Firmicutes phylum
...
# melted_obj was created in a separate R script. The data apart
# of melted_obj was loaded at the beginning of this file.
# This figure takes a closer look at Streptococcaceae in smokers
# according to the different environmental sample materials.
melted_obj %>%
  group_by(env_material, Phylum, smoker, Family) %>% # needed to group by Phylum, smoker,
  # env_material, and family because this figure only needs data in relation 
  # to these columns apart of the melted_obj table.
  ...
  # looking specifically at smoking individuals and dont need the data in
  # relation to the nonsmokers
  filter(Family == "Streptococcaceae") %>% # need to filter Streptococcaceae because 
  # we want to look at this family in particular, and not the others that are apart
...
melted_obj %>%
  group_by(env_material, Phylum, smoker, Family, host_subject_id) %>% # needed to 
